...
def get_sitemap(db: Session = Depends(get_db)):
    ...
    for story in stories:
        # Note: Story URL on frontend uses route param or query param?
        # Let's check frontend routing for stories: it's /stories/:slug or ?story=slug
        # Wait, the old sitemap used ?story=slug but in the test it was openStory(slug) which might be /stories/:slug?
        # Actually in StoriesGallery.jsx: "if (slug) openStory(slug);" -> Route is likely /stories/:slug
        # Let's just use /stories/:slug
        xml_content += f"  <url>\n"
        xml_content += f"    <loc>{base_url}/stories/{story.slug}</loc>\n"
        xml_content += f"    <lastmod>{story.created_at.strftime('%Y-%m-%d')}</lastmod>\n"
        xml_content += f"    <changefreq>weekly</changefreq>\n"
